...
- **Settings, split by scope** — two pages. **Profile** holds everything saved with a profile: the profile picker, name, length mode, intervals, pace and HR targets, warning cadence, audio cues, vibration and phase averages, plus baseline creation, profile export/import, *Save profile* and *Delete profile*. **General** holds the app-wide settings that no profile owns: appearance, sensors and debug — the **Debug** card (and its heading) stays hidden until the **General** tab is tapped `Constants.SETTINGS_DEBUG_UNLOCK_TAPS` (6) times in a row with nothing else pressed in between, and is revealed for as long as Settings stays open. The app-version footer sits under both.

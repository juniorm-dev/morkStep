...
> task run once. Three server quirks to know:
> - the **first** diagnostics request after a reload can report cascading `Unresolved
>   reference` false positives while the index warms — simply re-request; subsequent checks
>   are clean;
> - **code actions on a not-yet-opened file** return garbage quickfixes (e.g. N × "Import
>   `java.lang.reflect.Modifier`" / `jdk.javadoc…` / `com.sun…`): kotlin-lsp compiles
>   unopened documents without the project classpath, so every Compose/Android identifier
>   is "unresolved" and the quickfix provider offers every same-named JDK class (reproduced
>   on cold `MainViewModel`/`MorkApp`; clean after a diagnostics request warms the file).
>   Workaround: run diagnostics (or any open request) on the file before requesting code
>   actions;
> - **workspace symbol search is broken** in kotlin-lsp 1.3.13 (returns `Location` without
>   `range`, so the harness rejects the result); `typeDefinition`/`implementation` are
>   likewise unimplemented server-side.

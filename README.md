...
- **Profiles** — save unlimited named workout configurations; pick the active one on the **home screen** or in **Settings**, which also lists every saved profile and can **clone** or **delete** any of them. Starts from a single profile named *Default* with the **Adhoc** length.
...
- **Versioned APK artifact names.** AGP 9 removed `applicationVariants`/`BaseVariantOutputImpl`
  ...
  version goes after the build type (`morkStep-debug-0.14.3.apk`), and an unsigned release

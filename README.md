...
- **`WearPaceSource`** — pedometer cadence relayed from the paired **morkStep Wear** companion (path `/morkstep/pace`); the watch reads `STEPS_PER_MINUTE` via Wear Health Services and streams each steps-per-minute value. A **`PhonePaceSource`** falls back to the phone's own step sensor (`TYPE_STEP_DETECTOR`, else `TYPE_STEP_COUNTER`) when the watch is absent or silent for 15 s (`FallbackPaceSource`); the watch wins again instantly when it resumes. Phone cadence is derived over a rolling `Constants.PACE_WINDOW_MS` window (default 5 s).

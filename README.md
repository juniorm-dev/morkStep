...
| `[hc]` | Health Connect HR backfill: read outcome (window, buckets, phases), why a read was skipped/failed, and each retry or History sweep pass |
| `[gps]` | GPS speed fixes |
...
`app/src/test/java/com/morkstep/sensing/HealthConnectHrTest.kt` covers (10 tests): pooled push/recovery averaging from per-minute buckets, per-phase-occurrence bucketing (round attribution, buckets past the finish ignored, phases with no bucket skipped), and the merge that folds backfilled HR into a recorded phase list — filling only null HR, never overwriting a live reading, and ordering the union in workout order.

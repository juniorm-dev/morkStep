...
- **Workout history** — every completed session is auto-saved with **the profile it ran under** (date, duration, push count, distance *mi*, seconds above the push-min HR) plus **averages** at two levels: pooled per phase type (**push**, **recovery**, **overall**) and **per phase occurrence** — the warm-up, every push and recovery round, and the cool-down, each with its own speed/pace/HR. A History card is compact until tapped; opening it also re-reads Health Connect for that workout (see the Health Connect section) and adds the per-phase rows and a **line chart** of those phase averages, one line each for speed, pace and HR (a metric a workout never recorded draws no line, and a phase with no sample is a gap, not a zero). Averages are 1 Hz samples accumulated by the engine and bucketed per phase. The collapsed card's HR line reports whatever HR the row holds — the overall average when the read returned it, else the pooled push/recovery pair, else the min–max pair — so a partially backfilled row never reads as HR-less.
...
| `[hc]` | Health Connect HR backfill: SDK status + read permission, the provider probe (a record inside the window or none), read outcome (window, buckets, phases), why a read was skipped/failed, and each retry, History sweep or History-card re-read |
| `[gps]` | GPS speed fixes |
...
`app/src/test/java/com/morkstep/ui/HistoryCardHrTest.kt` covers (5 tests): the collapsed History card's HR line — the overall average, the pooled push/recovery fallback, a single pooled value, the min–max fallback, and silence when the workout recorded no HR at all.

...
- **Ads — test inventory only, off by default** — nothing is served until the hidden **Test ads (debug)** switch in Settings → General → Debug is turned on; with it off, no ad is requested, loaded or shown anywhere and the ad SDK is never even initialized. With it on, Google's **demo ad units** serve creatives labelled "Test Ad" under the sample AdMob app ID — there is no AdMob account attached and no live inventory. A second hidden switch, **Pinned ads (debug)**, picks the placement: **off** (the default) draws an anchored banner at the end of the Home column and a native card above the History list; **on** hosts the banner in the app's own bottom bar, above the tabs, so no screen's scrolling can carry it away and every screen shows the same pinned banner (on the **Workout** route that banner uses the smaller fixed 320×50 size, so the live session keeps its height), and gives the History screen a **full-page** native ad that opens on every third History access — the access count is persisted, so it resumes across an app restart — and stays until it is closed (its own **Close ad** button, or the system back gesture) — an inline History card is then replaced by that full-page ad rather than shown twice. Both history ads are closable, so no ad can trap the user on a screen. Ad lifecycle lines appear in the debug log under `[ads]`. What going live requires is recorded in `docs/ad-monetization-options.md`.
